...
Editing one of your messages swapped the bubble for a full-width bordered input with Cancel/Send buttons: the conversation jumped around you. The edit now happens inside the bubble itself, same shape, same colors, same place, with just a cursor and two small icon actions below it, the way review comments already edit in place. The keyboard works like the composer: Enter sends the edit, Shift+Enter adds a line, Escape cancels and puts the cursor back in the composer.
