import type { PullRequestResponse } from '../plugins/github/types.js'
import { beforeEach, describe, expect, it, vi } from 'vitest'

const fetchGithubPullRequests = vi.fn()
...
describe('github branch pull request routes', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  it('returns the open pull request for the requested branch', async () => {
    fetchGithubPullRequests.mockResolvedValue([makePullRequest()])

    const response = await request('/repos/acme/widget/pr/branch?branch=feature/parser')

    expect(response.status).toBe(200)
    expect(fetchGithubPullRequests).toHaveBeenCalledWith({
      token: 'github-token',
      params: {
        owner: 'acme',
        repo: 'widget',
  ...
  it('returns null when the branch has no open pull request', async () => {
    fetchGithubPullRequests.mockResolvedValue([])
  ...
  it('returns 400 when the branch query is missing', async () => {
    const response = await request('/repos/acme/widget/pr/branch')

    expect(response.status).toBe(400)
    await expect(response.json()).resolves.toEqual({
      error: 'Missing branch',
    })
    expect(fetchGithubPullRequests).not.toHaveBeenCalled()
  })
})
