...
const patchGithubPullRequest = vi.fn()
...
describe('github pull request base route', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    patchGithubPullRequest.mockResolvedValue({})
  ...
  it('updates the pull request base branch and invalidates PR diff caches', async () => {
    const response = await request('/pr/42/base?org=acme&repo=widget', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ base: 'release/next' }),
    })

    expect(response.status).toBe(204)
    expect(patchGithubPullRequest).toHaveBeenCalledWith({
      token: 'github-token',
      params: {
        owner: 'acme',
        repo: 'widget',
  ...
  it.each([403, 404, 422])(
    'passes through update base status %i from GitHub without invalidating cache',
    async (status) => {
      patchGithubPullRequest.mockRejectedValue(
        Object.assign(new Error(`GitHub update base error ${status}`), { status }),
      )
      ...
      await expect(response.json()).resolves.toEqual({
        error: `GitHub update base error ${status}`,
      })
    },
  )
})
