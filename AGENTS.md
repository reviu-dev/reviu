...
* `let _ =` is the idiom for `WeakEntity::update` and `cx.update_window`: a failure there means the entity is gone, not that something went wrong. On any other fallible `Result`, handle it:
  - Propagate with `?` when the caller should deal with it
  - `.log_err()` (from the `app_log` crate) when the error only needs to be visible in the log file
  - `match` / `if let Err(...)` when the failure needs custom logic
* Errors are only visible through `app_log`. `eprintln!` goes to a stderr nobody reads in a packaged desktop app, so keep it for CLI binaries (`reviu_driver`, examples) and tests only.
...
- **Keyboard handling**: use actions and keybindings for named user commands. Use raw key handlers only for local interaction details such as cancelling inline edits or stopping propagation in a transient state.
- **Empty and loading states**: use skeletons for loading when the final content shape is known, such as lists, trees, or rows. Use an explanatory card for true empty, blocked, or missing-setup states where the user needs context or a next step.
