//! SGR-aware ANSI handling for terminal output: colors and emphasis become
//! styled spans, every other escape sequence is stripped.
...
pub(crate) struct AnsiLine {
  ...
  pub text: String,
  /// Styled ranges into `text`; gaps render with the default style.
...
/// restarts its line (progress bars show their final state).
pub(crate) fn parse_ansi(input: &str) -> Vec<AnsiLine> {
  ...
  while let Some(ch) = chars.next() {
    match ch {
      '\u{1b}' => match chars.peek() {
        Some('[') => {
          ...
          for c in chars.by_ref() {
            match c {
              ...
              // Malformed sequence: stop consuming, drop what was read.
              _ => break,
            }
          }
        ...
        // ESC + intermediate (charset designation like `ESC ( B`): the
        // intermediate carries one final byte after it.
      ...
      '\r' => {
        ...
        // Overwrite from column zero: keep only what comes after.
        line.text.clear();
        line.spans.clear();
        span_start = 0;
      }
