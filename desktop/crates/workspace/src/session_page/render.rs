...
const CENTER_DROP_EDGE_FRACTION: f32 = 0.33;
const CENTER_DROP_STICKY_EDGE_FRACTION: f32 = 0.40;
...
mod tests {
  ...
  async fn closing_a_terminal_tab_drops_its_shell(cx: &mut TestAppContext) {
    ...
    page.update_in(cx, |page, window, cx| page.new_terminal_tab(window, cx));
    cx.run_until_parked();
    assert!(ConfigStore::load_center_workspace(&repo.path).is_some());
    page.update_in(cx, |page, window, cx| {
    ...
    assert_eq!(ConfigStore::load_center_workspace(&repo.path), None);
  }
  ...
  async fn forgetting_a_project_drops_its_terminal_tabs(cx: &mut TestAppContext) {
    ...
    ConfigStore::persist_center_workspace(&unloaded_checkout, &repo.path, "{}");
    ...
    assert_eq!(ConfigStore::load_center_workspace(&unloaded_checkout), None);
  }
  ...
  #[gpui::test]
  async fn selecting_a_sidebar_chat_restores_the_split_tab_group(cx: &mut TestAppContext) {
    let repo = TempRepo::init("session-sidebar-chat-restores-split");
    commit_text_file(&repo.path, Path::new("analytics.ts"), "one\n", "initial");
    let (page, cx) = add_session_page_window(repo.path.clone(), cx);
    cx.run_until_parked();
    ...
    page.read_with(cx, |page, _| {
      ...
      assert!(!page.center_tabs.contains(&first_chat_tab));
    });
    assert!(cx.debug_bounds("session-conversation-pane").is_some());
    assert!(cx.debug_bounds("session-diff-editor").is_some());
  }
