    ...
    for segment in split_details_segments(source) {
        match segment {
            ...
            Segment::Details {
                ...
                let summary_inlines =
                    summary_inlines_from_text(summary.as_deref().unwrap_or("Details"));
...
pub(crate) fn split_details_line(
    line: &str,
    mut depth: isize,
) -> (String, Option<String>, isize) {
    let lower = line.to_ascii_lowercase();
    let mut idx = 0usize;
    ...
    match &node.data.borrow().value {
        ...
        NodeValue::HtmlBlock(html) => {
            if is_html_comment_only_block(&html.literal)
                || is_details_close_only_block(&html.literal)
            {
                Vec::new()
...
pub(crate) fn inlines_from_nodes<'a>(
...
pub(crate) fn inline_to_plain_text(inlines: &[Inline]) -> String {
    ...
    for inline in inlines {
        match inline {
            ...
            | Inline::Strikethrough(children) => text.push_str(&inline_to_plain_text(children)),
